...
export default function SearchPage() {
  ...
  useEffect(() => {
    fetch("/api/prices")
      .then((r) => r.json())
      .then((data) => { setAllPrices(data); setLoading(false); })
  ...
  return (
    <div className="flex flex-col min-h-screen">
            ...
            <div className="space-y-2">
              {results.map((p) => (
                <Card key={p.id}><CardContent className="p-4 flex items-center justify-between">
                  ...
                  <VndPrice amount={p.price_vnd} className="text-base shrink-0 ml-3" />
                </CardContent></Card>
              ))}
            </div>
          ) : query && results.length === 0 ? (
      ...
      <PublicFooter />
      <FloatingContact zaloLink={DEFAULT_ZALO_LINK} facebookLink={DEFAULT_FACEBOOK_LINK} />
    </div>
  );
}
